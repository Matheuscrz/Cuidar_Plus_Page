...
export default function Product({ onOpenModal }: ProductProps) {
  ...
  return (
    <section id="produto" className="py-24 px-6 bg-[#1FA0C9] text-white overflow-hidden relative">
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center relative z-10">
        <div>
          ...
          <h2 className="text-3xl md:text-5xl font-bold mb-6 leading-tight">Leve o cuidado para casa.</h2>
          <p className="text-white/90 text-lg mb-8 leading-relaxed">
            O Kit Cuidar+ inclui o <strong>dispositivo de monitorização</strong> inteligente e acesso vitalício à aplicação móvel Premium. Segurança e autonomia em um só lugar.
          </p>
          
          <ul className="space-y-4 mb-10">
            {['Sincronização Bluetooth 5.0', 'Bateria com 7 dias de autonomia'].map((feat) => (
              <li key={feat} className="flex items-center gap-3">
                <div className="flex-shrink-0 w-6 h-6 rounded-full bg-white text-[#1FA0C9] flex items-center justify-center text-xs font-bold">✓</div>
        ...
        <div className="relative h-[450px] w-full bg-white/10 rounded-3xl backdrop-blur-sm border border-white/20 flex items-center justify-center p-8 shadow-2xl">
          <div className="text-center">
            <p className="text-white/60 text-sm mb-6 tracking-widest uppercase">Visualização do Hardware</p>
            {/* Simulação do Produto com animação */}
            <div className="relative w-64 h-64 mx-auto flex items-center justify-center">
                 <div className="absolute inset-0 bg-[#1FA0C9] rounded-full blur-2xl opacity-40 animate-pulse"></div>
                 <div className="w-56 h-56 bg-slate-900 rounded-full border-8 border-slate-800 shadow-2xl flex items-center justify-center relative z-10">
